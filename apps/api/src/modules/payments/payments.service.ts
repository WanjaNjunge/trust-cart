...
export class PaymentsService {
  ...
  async initiatePayment(userId: string | undefined, dto: InitiatePaymentDto) {
    ...
    // Idempotency: return existing non-failed transaction for this order
    ...
    if (existingTransaction) {
      this.logger.log(
        `Idempotency hit: returning existing transaction ${existingTransaction.id} (${existingTransaction.status})`,
      );
      return {
        status: existingTransaction.status,
        message:
          existingTransaction.status === PaymentStatus.CONFIRMED
  ...
  async getPayment(
    ...
    const transaction = await this.prisma.paymentTransaction.findUnique({
      ...
      include: {
        ...
        mpesaTransaction: { select: { mpesaReceipt: true } },
      },
    });
    ...
    const isOwner = transaction.order.userId && transaction.order.userId === requestingUserId;
    const isStaff =
      requestingUserRole && ['STAFF', 'MANAGER', 'ADMIN'].includes(requestingUserRole);
    ...
    return {
      ...
      mpesaReceiptNumber: transaction.mpesaTransaction?.mpesaReceipt ?? null,
      createdAt: transaction.initiatedAt,
      confirmedAt: transaction.confirmedAt,
    };
  }

  private async handlePoDInit(order: OrderForPayment) {
    // 1. Validate Limit
    if (order.total > this.POD_LIMIT) {
      throw new BadRequestException(
        `Pay on Delivery is limited to KES ${this.POD_LIMIT.toLocaleString()}`,
      );
    }
    ...
    if (order.address && !allowedCounties.includes(order.address.county)) {
      ...
      if (!allowedCounties.includes(order.address.county)) {
        throw new BadRequestException(
          'Pay on Delivery is only available in Nairobi and surrounding areas',
        );
      }
    }
  ...
  private async handleMpesaInit(order: OrderForPayment, _phoneNumber?: string) {
    // STUB IMPLEMENTATION
