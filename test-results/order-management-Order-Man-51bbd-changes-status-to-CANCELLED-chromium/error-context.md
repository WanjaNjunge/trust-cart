...
- Name: order-management.spec.ts >> Order Management (Customer) >> cancel order changes status to CANCELLED
- Location: e2e\order-management.spec.ts:67:7
...
Locator: getByRole('button', { name: /cancel order/i })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /cancel order/i })
